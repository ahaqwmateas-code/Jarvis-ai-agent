...
export class AudioSessionManager {
  ...
  constructor(audioFeedback: boolean = true) {
    ...
    if (NativeAudioRecorder.isAvailable()) {
      ...
      // FFmpeg fallback below uses -f avfoundation (macOS-only), so
      // Windows/Linux go straight to the cross-platform recorder.
      try {
        Logger.info('🚀 [Audio] Using cross-platform audio recording (naudiodon2)');
        this.audioRecorder = new CrossPlatformAudioRecorder();
      } catch (error) {
        Logger.error('⚠️ [Audio] Cross-platform recorder failed, falling back to FFmpeg:', error);
