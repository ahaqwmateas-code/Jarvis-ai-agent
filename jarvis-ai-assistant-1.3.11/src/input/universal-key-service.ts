...
import { CrossPlatformKeyMonitor } from './cross-platform-key-monitor';
...
export class UniversalKeyService {
  ...
  private loadNativeModule(): boolean {
    ...
    if (process.platform !== 'darwin') {
      try {
        this.keyMonitor = new CrossPlatformKeyMonitor();
        Logger.success('Cross-platform key monitor (uiohook) loaded successfully');
        return true;
      } catch (error) {
        Logger.error('Failed to load cross-platform key monitor:', error);
        return false;
      }
    }
